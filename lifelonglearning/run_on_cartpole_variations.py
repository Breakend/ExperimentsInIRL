...
for i in range(1, args.num_variations+1):
    gym.envs.register(
        id="Custom-PoleLength-v%d" % i,
        entry_point="envs.transfer.classic.cartpole:CustomizableCartPoleEnv",
        max_episode_steps=200,
        reward_threshold=195.0,
        kwargs= dict(length=.1*i)
    )
...
gymenv =  GymEnv("CartPole-v0", force_reset=True)
...
if type(env.spec.action_space) is Discrete:
    ...
    raise Exception("Gaussian decomposed not supported yet")
...
with tf.Session() as sess:
    ...
    for i in range(1, args.num_variations+1):
        print("Variation %d" % i)
        gymenv =  GymEnv("Custom-PoleLength-v%d" % i, force_reset=True)
        env = TfEnv(normalize(gymenv, normalize_obs=False))
